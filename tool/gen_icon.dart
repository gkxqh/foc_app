...
void main() {
  ...
  final main = img.copyResize(src, height: 1024, interpolation: img.Interpolation.cubic);
  ...
  final fg = img.copyResize(src, height: (1024 * 0.66).round(), interpolation: img.Interpolation.cubic);
  final fgCanvas = img.Image(width: 1024, height: 1024, numChannels: 4);
  img.compositeImage(fgCanvas, fg, dstX: (1024 - fg.width) ~/ 2, dstY: (1024 - fg.height) ~/ 2);
  File('assets/icon/icon_foreground.png').writeAsBytesSync(img.encodePng(fgCanvas));
